...
module.exports = class VulnReportKitPlugin extends Plugin {
	async onload() {
		...
		if (this.settings.interfaceMode === 'advanced') {
			this.addCommand({
				id: 'create-demo-case',
...
class VulnReportKitSettingTab extends PluginSettingTab {
	...
	display() {
		...
		containerEl.createEl('h3', { text: 'Interface mode' });
		new Setting(containerEl)
			.setName('Mode')
			.setDesc('Light mode shows the essential workflow commands. Advanced mode enables template management, exports, backups, demo utilities, and extra helpers. Reload the plugin after changing this setting.')
			.addDropdown((dropdown) => dropdown
				.addOptions({ light: 'Light', advanced: 'Advanced' })
				.setValue(this.plugin.settings.interfaceMode || DEFAULT_SETTINGS.interfaceMode)
				.onChange(async (value) => {
					this.plugin.settings.interfaceMode = value;
					await this.plugin.saveSettings();
					new Notice('Interface mode saved. Reload the plugin to refresh visible commands.');
				}));
		...
		new Setting(containerEl)
			...
			.setDesc('Vault folder used by the import command. Extract custom or advanced template packs here before importing.')
			.addText((text) => text
				.setPlaceholder(DEFAULT_SETTINGS.defaultTemplatePackSource)
...
function buildHealthCheckReport(data) {
	...
	lines.push(`Interface mode: ${data.settings.interfaceMode || 'light'}`);
	lines.push('');
	...
	if (!(data.templates || []).length) {
		lines.push('- Install the starter template pack or import a advanced or custom template pack.');
	}
...
## Importing a custom or advanced template pack
